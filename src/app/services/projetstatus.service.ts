...
export class ProjetstatusService {
  ...
  approveProject(id:number, reason:string):Observable<any>{
    return this.http.patch<any>(`https://dschangschoolhub.ddns.net/api/usecases/status/approved/pending/${id}`,
      {motif:reason}).pipe(
  ...
  rejectProject(id:number, reason:string):Observable<any>{
    ...
    return this.http.patch<any>(`https://dschangschoolhub.ddns.net/api/usecases/status/rejected/pending/${id}`,
      { motif: reason }
    ).pipe(
      tap(() => this.projetService.notifyProjectChanged(id))
    );
    ...
    return this.http.patch<any>(`https://dschangschoolhub.ddns.net/api/usecases/status/pending/${id}`,
      {motif:reason}).pipe(
      tap(() => this.projetService.notifyProjectChanged(id))
    );
  }

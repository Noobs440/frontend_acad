...
export class DocumentService {
  ...
  private baseUrl = 'https://uds-faculte-des-sciences.netlify.app//api/ressources/documents';
  ...
  return this.http.post<any>('https://uds-faculte-des-sciences.netlify.app//api/ressources/documents', formData);
}

updateDocumentMultipart(id: number, formData: FormData) {
  formData.append('_method', 'PUT'); // important !
  return this.http.post<any>(`https://uds-faculte-des-sciences.netlify.app//api/ressources/documents/${id}`, formData);
}
  getDocumentsByProject(id:number): Observable<any[]>{
    return this.http.get<any[]>(`https://uds-faculte-des-sciences.netlify.app//api/usecases/listing/projet/documents/${id}`).pipe(
